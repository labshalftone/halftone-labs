        ...
        if (currentUser) {
          await fetch("/api/affiliate/apply", {
            ...
            body: JSON.stringify({
              ...
              name: data.brand_name || data.store_name || currentUser.email?.split("@")[0] || "User",
...
function Step8Success({
  ...
  useEffect(() => {
    // Fetch affiliate code that was auto-created
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      fetch(`/api/affiliate/stats?userId=${user.id}`)
        .then((r) => r.json())
        .then((d) => { if (d.affiliate?.code) setRefCode(d.affiliate.code); })
        .catch(() => {});
    });
  }, []);
  ...
  const copyRef = () => {
    if (!refCode) return;
    navigator.clipboard.writeText(`https://halftonelabs.in?ref=${refCode}`);
    setRefCopied(true);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-zinc-50 border border-black/[0.06] mb-4">
      ...
      {/* Affiliate welcome */}
      <div className="rounded-xl border border-black/[0.06] bg-gradient-to-br from-orange-50 to-white p-4 mb-6">
        <div className="flex items-center gap-2 mb-2">
          <svg className="w-4 h-4 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
          </svg>
          <p className="text-xs font-bold text-orange-600 uppercase tracking-widest">You&apos;re now an affiliate</p>
        </div>
        <p className="text-xs text-zinc-500 mb-3 leading-relaxed">
          Share your referral link and earn <strong className="text-zinc-700">5% per order</strong>, up to <strong className="text-zinc-700">₹5,000 per subscription</strong>, and <strong className="text-zinc-700">10% monthly for 12 months</strong>.
        </p>
        {refCode ? (
          <button
            onClick={copyRef}
            className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-white border border-black/[0.08] hover:border-black/20 transition-colors text-left"
          >
            <span className="text-xs font-mono text-zinc-600 truncate">halftonelabs.in?ref={refCode}</span>
            <span className="text-xs font-semibold text-orange-500 shrink-0">{refCopied ? "Copied!" : "Copy"}</span>
          </button>
        ) : (
